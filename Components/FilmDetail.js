...
class FilmDetail extends React.Component {
  ...
  componentDidMount() {
    ...
    const seenFilmIndex = this.props.seensFilm.findIndex(item => item.id === this.props.navigation.state.params.idFilm)
    if (seenFilmIndex !== -1) {
      this.setState({
        film: this.props.seensFilm[seenFilmIndex]
      }, () => { this._updateNavigationParams() })
      return
    }
  ...
  _displayButtonSeen() {
    ...
    if (this.props.seensFilm.findIndex(item => item.id === this.props.navigation.state.params.idFilm) !== -1) {
      return (
        <Button
